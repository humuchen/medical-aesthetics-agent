...
import { PRESET_ICONS, PRESET_COLORS, PRESET_TEMPLATES } from '../constants/agent';
import { MODEL_PROVIDERS, DEFAULT_BASE_URL } from '../constants/model';
import { PERMISSION_MODES } from '../constants/permission';
import { API_PATHS, EXTERNAL_URLS } from '../constants/api';
...
export function SettingsPage({ 
  ...
  const [defaultAgentId, setDefaultAgentId] = useState(() => 
    localStorage.getItem(STORAGE_KEYS.defaultAgentId) || 'default'
  );
  const [defaultCwd, setDefaultCwd] = useState(() => 
    localStorage.getItem(STORAGE_KEYS.defaultCwd) || ''
  );
  ...
  const checkLoginStatus = useCallback(async () => {
    ...
    try {
      const response = await fetch(API_PATHS.checkLogin);
  ...
  const saveEnvConfig = async () => {
    ...
    setSavingEnv(true);
    try {
      const response = await fetch(API_PATHS.saveEnvConfig, {
        method: 'POST',
  ...
  const handleSaveModel = async () => {
    ...
    try {
      if (editingModel) {
        await onUpdateCustomModel(editingModel.id, modelFormData);
        MessagePlugin.success('自定义模型已更新');
      } else {
        await onAddCustomModel(modelFormData);
  ...
  const fetchDbStats = useCallback(async () => {
    ...
    try {
      const res = await fetch(API_PATHS.dbStats);
  ...
  const handleClearDb = useCallback(async () => {
    try {
      const res = await fetch(API_PATHS.dbClear, { method: 'DELETE' });
      const data = await res.json();
      if (data.success) {
  ...
  const handleSaveDefaultConfig = () => {
    localStorage.setItem(STORAGE_KEYS.defaultAgentId, defaultAgentId);
    localStorage.setItem(STORAGE_KEYS.defaultCwd, defaultCwd);
        ...
        <div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link 
                href={EXTERNAL_URLS.codebuddyCliDocs}
                target="_blank"
                theme="primary"
                size="small"
              >
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--td-text-color-secondary)' }}>
                          提供商 <span style={{ color: 'var(--td-error-color)' }}>*</span>
                        </label>
                        <Select
                      ...
                      <div>
                        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--td-text-color-secondary)' }}>
                          模型 ID <span style={{ color: 'var(--td-error-color)' }}>*</span>
                        </label>
                        <Input
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--td-text-color-secondary)' }}>
                          显示名称 <span style={{ color: 'var(--td-error-color)' }}>*</span>
                        </label>
                        <Input
                    ...
                    <div>
                      <label className="text-sm font-medium block mb-1" style={{ color: 'var(--td-text-color-secondary)' }}>
                        Base URL <span style={{ color: 'var(--td-error-color)' }}>*</span>
                      </label>
                      <Input
                    ...
                    <div>
                      <label className="text-sm font-medium block mb-1" style={{ color: 'var(--td-text-color-secondary)' }}>
                        API Key {!editingModel && <span style={{ color: 'var(--td-error-color)' }}>*</span>}
                      </label>
                      <Input
                        type="password"
